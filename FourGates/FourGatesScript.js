...
$(".moladDiv").on("click", function() {
  ...
  mvPtr(molad2,2); // adjust pointer for second ruler
  mvOtherPtrs(molad2);
...
$(".moladCI").on("input",function(){ // update all pointers and counters on input
  ...
  //mvPtr(molad2,2,true); // adjust pointer for second ruler
...
function mvOtherPtrs(molad2) { // only pointer 2 is draggable. Move the rest.
...
function mvPtr(molad2,ii,noctrf) { // other rulers 1 3 or 4, move to match appropriately
  ...
  //let newtop1 = getPlace(molad2decimal(otherMolad),ii);
  //$("#ptr_"+ii).css("top",newtop1);
